...
router.get('/coding-hours', authenticate, async (req, res) => {
    const userId = req.user.id;
...
router.get('/hours-distribution', authenticate, async (req, res) => {
    const userId = req.user.id;
    try {
        const { data, error } = await supabaseAdmin
            .from('activities')
...
router.get('/personal-bests', authenticate, async (req, res) => {
    const userId = req.user.id;
    try {
