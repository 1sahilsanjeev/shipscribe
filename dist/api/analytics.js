...
router.get('/heatmap', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
    const days = parseInt(req.query.days) || 364;
...
router.get('/stats', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
...
router.get('/coding-hours', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
...
router.get('/hours', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
...
router.get('/projects', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
    // 1. Get project hours
    ...
    (hoursData || []).forEach(d => {
        ...
        if (!projectStats[p])
            projectStats[p] = { project: p, hours: 0, commits: 0, tasks: 0 };
    ...
    (taskData || []).forEach(d => {
        ...
        if (!projectStats[p])
            projectStats[p] = { project: p, hours: 0, commits: 0, tasks: 0 };
        if (d.status === 'done')
            projectStats[p].tasks += 1;
    });
    const result = Object.values(projectStats).sort((a, b) => b.hours - a.hours);
    res.json(result);
}));
// GET /api/analytics/projects-list
router.get('/projects-list', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
...
router.get('/streak', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
...
router.get('/productivity', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
    ...
    const result = Object.entries(dailyData).map(([date, val]) => {
...
router.get('/hours-distribution', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
    const { data, error } = await supabaseAdmin
        .from('activities')
        .select('timestamp')
...
router.get('/personal-bests', authenticate, handle(async (req, res) => {
    const userId = req.user.id;
    ...
    Object.entries(commitGroups).forEach(([date, val]) => {
        if (val > maxCommits.value)
            maxCommits = { value: val, date: date };
    });
    ...
    Object.entries(hourGroups).forEach(([date, val]) => {
        if (val > maxHours.value)
            maxHours = { value: Number(val.toFixed(1)), date: date };
    });
    ...
    Object.entries(taskGroups).forEach(([date, val]) => {
        if (val > maxTasks.value)
            maxTasks = { value: val, date: date };
    });
    ...
    Object.entries(dailyData).forEach(([date, val]) => {
        ...
        if (score > maxScore.value)
            maxScore = { value: score, date: date };
    });
    res.json({
        commits: maxCommits,
        hours: maxHours,
