    ...
    try {
        switch (name) {
            ...
            case "get_active_session": {
                ...
                if (!session)
                    return res.json({ content: [{ type: "text", text: "No active session found." }] });
            ...
            case "get_current_session": {
                ...
                if (!result)
                    return res.json({ content: [{ type: "text", text: "No active live session found." }] });
                const md = [
            ...
            case "sync_github": {
                ...
                if (result.error)
                    return res.json({ content: [{ type: "text", text: `Error: ${result.error}` }] });
                const md = [
                    `## GitHub Sync ✓`,
                    ``,
...
router.post('/file_change', async (req, res) => {
