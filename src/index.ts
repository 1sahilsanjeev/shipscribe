  ...
  try {
    switch (name) {
      ...
      case "get_current_session": {
        ...
        if (!result) return { content: [{ type: "text", text: "No active live session found." }] };
        const md = [
      ...
      case "debug_env": {
        ...
        const relevant = allEnvKeys.filter(key => 
          key.toLowerCase().includes('antigravity') ||
