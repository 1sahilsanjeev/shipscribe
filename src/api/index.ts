...
app.get('/api/activities', authenticate, async (req: AuthenticatedRequest, res) => {
...
app.get('/api/activities/export', authenticate, async (req: AuthenticatedRequest, res) => {
...
app.get('/api/activities/today', authenticate, async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
...
app.get('/api/activity/live', authenticate, async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
  try {
...
app.patch('/api/tasks/:id/status', authenticate, async (req: AuthenticatedRequest, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (error) return res.status(404).json({ error: 'Task not found or unauthorized' });
    res.json(data);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
