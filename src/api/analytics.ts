...
router.get('/heatmap', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  ...
  if (error) {
    ...
    const { data: rawData, error: rawError } = await supabaseAdmin
      ...
      .select('timestamp')
      .eq('user_id', userId)
      .gte('timestamp', new Date((Date.now() - days * 24 * 60 * 60 * 1000) as any).toISOString());
...
router.get('/stats', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
  const today = new Date().toISOString().split('T')[0];
  const startOfWeek = new Date((Date.now() - 7 * 24 * 60 * 60 * 1000) as any).toISOString();
...
router.get('/coding-hours', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
  const days = parseInt(req.query.days as string) || 7;
  const startDate = new Date((Date.now() - days * 24 * 60 * 60 * 1000) as any).toISOString();
...
router.get('/hours', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
...
router.get('/projects', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
...
router.get('/projects-list', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
...
router.get('/streak', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
  ...
  const today = new Date().toISOString().split('T')[0];
  const yesterday = new Date((Date.now() - 86400000) as any).toISOString().split('T')[0];
  ...
  if (dates[0] === today || dates[0] === yesterday) {
    ...
    for (let i = 0; i < dates.length - 1; i++) {
      const curr = new Date(dates[i] as any);
      const next = new Date(dates[i+1] as any);
...
router.get('/productivity', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
  ...
  for (let i = 0; i <= days; i++) {
    const d = new Date((Date.now() - i * 24 * 60 * 60 * 1000) as any).toISOString().split('T')[0];
...
router.get('/hours-distribution', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select('timestamp')
    .eq('user_id', userId)
    .gte('timestamp', new Date((Date.now() - 30 * 24 * 60 * 60 * 1000) as any).toISOString());

  if (error) throw error;

  const distribution = Array(24).fill(0);
  (data || []).forEach(a => {
    const hour = new Date(a.timestamp as any).getUTCHours();
    distribution[hour]++;
  });
...
router.get('/personal-bests', authenticate, handle(async (req: AuthenticatedRequest, res) => {
  const userId = req.user.id;
  ...
  const startDate = new Date((Date.now() - 90 * 24 * 60 * 60 * 1000) as any).toISOString();
  const [tasks, sessions, activity] = await Promise.all([
    supabaseAdmin.from('tasks').select('completed_at').eq('user_id', userId).eq('status', 'done').gte('completed_at', startDate),
    supabaseAdmin.from('time_sessions').select('start_time, duration_mins').eq('user_id', userId).gte('start_time', startDate),
    supabaseAdmin.from('activities').select('timestamp').eq('user_id', userId).eq('source', 'github').gte('timestamp', startDate)
