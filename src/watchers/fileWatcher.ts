...
export async function startFileWatcher(apiKey: string, apiUrl: string) {
  ...
  fs.watch(cwd, { recursive: true }, async (eventType, filename) => {
    ...
    if (parts.some(part => IGNORE_DIRS.includes(part))) return;
    if (IGNORE_FILES.includes(path.basename(filename))) return;
    if (IGNORE_EXTENSIONS.includes(path.extname(filename))) return;
...
export async function getActiveSession(userId: string, projectName: string) {
